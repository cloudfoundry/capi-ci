...
pushd capi-release > /dev/null
  current_jemalloc_blob_fullname=$(grep -m 1 "jemalloc" config/blobs.yml | awk -F':' '{print $1}') || { echo "Error: grep command failed."; exit 1; }
  current_jemalloc_version=$(echo "${current_jemalloc_blob_fullname}" | awk -F'/' '{print $2}' | awk -F'jemalloc-' '{print $2}' | awk -F'.tar.bz2' '{print $1}') || { echo "Error: awk command failed."; exit 1; }
